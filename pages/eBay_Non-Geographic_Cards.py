...
st.set_page_config(
    page_title="eBay Non-Geographic Processor",
    page_icon="🖼️",
...
def submit_processing_job(links):
    ...
    endpoint = f"{BACKEND_API_URL}/nongeo/process-postcards"
    payload = {"links": links}
    try:
...
def submit_job_page():
    ...
    if st.session_state.reset_after_submit_at is not None:
        ...
        if remaining_time <= 0:
            ...
            st.info(
                f"Job submitted successfully. You can navigate to 'View Results' now.")
